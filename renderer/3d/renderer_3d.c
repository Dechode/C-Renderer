...
void initRenderer3D(RenderState *state, float vFov, float near, float far) {
  ...
  initCamera3D(&_internalState->camera, (vec3){0.0f, 2.0f, 0.0f},
...
void draw(Shader *shader, uint32_t vao, uint32_t indexCount, mat4x4 transform) {
  ...
  setMat4(shader, "view", _internalState->camera.view);
...
void drawCube(Material *material, vec3 position, vec3 rotation, float angle,
  ...
  setFloat3(material->shader, "viewPos",
            _internalState->camera.position);
  ...
  setMat4(material->shader, "projection",
          _internalState->camera.projection);

  setFloat3(material->shader, "directionalLight.direction",
         _state3D.sun.direction);
  setFloat3(material->shader, "directionalLight.diffuse",
         _state3D.sun.diffuse);
  setFloat3(material->shader, "directionalLight.ambient",
         _state3D.sun.ambient);
